...
import AnuaLogo from '../assets/anua/anua_logo.webp';
import product1 from '../assets/anua/img_1277.webp';
import product2 from '../assets/anua/peach-70-niacinamide-serum.jpg';
import product3 from '../assets/anua/anua-7-rice-ceramide-hydrating-barrier-serum-50ml.avif';
import product4 from '../assets/anua/Anua_Essentials_Bestsellers_Kit_1.webp';
import BlueTick from '../assets/blue-checkmark-validation-social-media-png.png';
import { Fragment } from 'react';
import { Link } from 'react-router-dom';

function Anua() {
  const AnuaCollection = [
    {
      id: 'anua001',
      fullPrice: 14.99,
  ...
  // product count
  const ProductCount = AnuaCollection.length;
            ...
            >
              ...
              <div className="relative w-full h-max flex justify-center items-center">
                ...
                <div className="absolute left-0 bottom-0">
                  {getDeliveryStatus(anua.deliveryStatus)}
                </div>
              </div>

              {/* Info */}
              <div className="h-full pl-2.5 pt-2.5 ">
                <h1 className="text-lg font-medium truncate">{anua.title}</h1>
                ...
                <div className="flex items-center">
                  ...
                  <p className="text-[12px] text-gray-600/70">
                    {formatCount(anua.sold)} sold
                  </p>

                  {/*Dynamic Stock Status */}
                </div>
                {stockStatus(anua.stock)}
                {/* delivery */}
                {/* {getDeliveryStatus(anua.deliveryStatus)} */}
                {/* Price */}
                <div className="flex gap-3">
                  <p className="text-2xl text-red-500 font-medium">
                    ${getDiscountPrice(anua.discountPercent, anua.fullPrice)}
                  </p>
              ...
              <div className="absolute top-2 left-2 h-6.5 w-12 bg-[var(--primary-color)] flex justify-center items-center">
                <span className="text-white text-sm">
                  -{anua.discountPercent}%
                </span>
              </div>
              {/* botton */}
              <button
                disabled={anua.stock === 0}
